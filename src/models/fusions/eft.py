"""
Early Fusion Transformer (EFT) — concatenate modality tokens along time axis,
then process through a shared Transformer.

早期融合 Transformer — 沿时间轴拼接各模态 token，再用共享 Transformer 处理。

Architecture:
    For each modality:  tokens → pos_encoding → modality_embedding
    Concatenate all tokens along time dim → Transformer Encoder × N → Pooling

Cross-modal interaction starts from the FIRST Transformer layer via self-attention.
"""
...
class EFTFusion(BaseFusion):
    ...
    concatenates them along time dimension, and passes through a shared
    Transformer encoder. Cross-modal interaction occurs from layer 1.
    ...
    def _prepare_tokens_and_masks(
        ...
        # Default EFT path: concatenate modality tokens along time dimension.
